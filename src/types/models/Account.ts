...
export class Account implements CompatEntity {
    ...
    public id: string;
    public firstSeenLedger?: number;
    public lastSeenLedger?: number;
     ...
     * */
    static async getByFields(filter: FieldsExpression<AccountProps>[], options: GetOptions<CompatAccountProps>): Promise<Account[]> {
        const records = await store.getByFields<CompatAccountProps>('Account', filter, options);
